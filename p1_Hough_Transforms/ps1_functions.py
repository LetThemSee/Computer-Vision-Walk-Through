...
def hough_peaks(H, n_peaks_atmost, threshold_factor=None, hood_size=None):
    if threshold_factor is None:
        threshold = 0.5 * np.max(H)
    else:
        threshold = threshold_factor * np.max(H)
    n_row, n_col = H.shape
...
def find_circles(img_edges, radius_range, img_rgb):
    # 1. Hough accumulator
    theta_range = range(-180, 180)
    ...
    half_hood_row = int(height / 100) # hood_row = int(n_row/100) * 2 + 1 !Default
    half_hood_col = int(width / 100) # hood_col = int(n_col/100) * 2 + 1 !Default
